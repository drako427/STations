...
import { ShieldCheck, Globe, Users, FileBarChart, Siren, Building2, Activity, AlertTriangle, TrendingUp, FileText, UserCheck, Scale } from "lucide-react";

interface DpoDashboardCacheEntry {
    token: string;
    fetchedAt: number;
    data: any;
}

const DPO_DASHBOARD_CACHE_TTL_MS = 5 * 60 * 1000;
let dpoDashboardCache: DpoDashboardCacheEntry | null = null;
...
export default function DPODashboard() {
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            try {
                ...
                const hasValidCache =
                    dpoDashboardCache &&
                    dpoDashboardCache.token === token &&
                    Date.now() - dpoDashboardCache.fetchedAt < DPO_DASHBOARD_CACHE_TTL_MS;

                if (hasValidCache) {
                    setData(dpoDashboardCache.data);
                    setError(null);
                    return;
                }
    ...
    return (
        <div className="space-y-8 animate-in">
            {error && data && (
                <div className="rounded-lg border border-amber-400/30 bg-amber-400/10 px-4 py-2 text-xs font-medium text-amber-300">
                    Using cached dashboard data. Update failed: {error}
                </div>
            )}
