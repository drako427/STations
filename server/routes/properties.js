...
async function fetchPropertiesByStationId(stationId) {
...
/**
 * GET /api/properties
 * Retrieve all properties for the authenticated user's own station.
 */
router.get('/', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const rows = await fetchPropertiesByStationId(stationId);
...
/**
 * GET /api/properties/network
 * Read-only cross-station lookup for the national property page.
 */
router.get('/network', authenticateToken, async (req, res) => {
    const requestedStationId = parseInt(req.query.station_id, 10);
    ...
    try {
        console.log(`🔍 Network property lookup for station_id: ${requestedStationId}, requested by user: ${req.user.user_id}, home station: ${req.user.station_id}`);
        const rows = await fetchPropertiesByStationId(requestedStationId);
        res.status(200).json(rows);
    } catch (error) {
        console.error('❌ Error fetching network properties:', error);
        res.status(500).json({ error: 'Failed to fetch network properties' });
    }
});
