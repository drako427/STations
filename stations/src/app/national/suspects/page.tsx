...
export default function NationalSuspectsPage() {
    ...
    const fetchSuspectCounts = async (stationsList: Station[], token: string) => {
        ...
        for (let i = 0; i < stationsList.length; i++) {
            ...
            try {
                ...
                const response = await fetch(`/api/suspects/station/${station.station_id}`, {
                    headers: {
                        'Content-Type': 'application/json',
    ...
    const fetchStationSuspects = async (stationId: number) => {
        ...
        try {
            ...
            const response = await fetch(`/api/suspects/station/${stationId}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
    ...
    const fetchAllSuspects = async () => {
        ...
        try {
            ...
            // Decode token to get user's station_id
            const tokenPayload = JSON.parse(atob(token.split('.')[1]));
            const userStationId = tokenPayload.stationId; // Changed from station_id to stationId

            const response = await fetch(`/api/suspects/station/${userStationId}`, {
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
                }
